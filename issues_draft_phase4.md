# Phase 4 — Wave Issue Drafts

Wave cycle: 7 days · Complexity labels: Trivial (100pts) / Medium (150pts) / High (200pts)

---

## Issue 1 — Enforce RP ID and Origin Verification in `__check_auth`
...
- Soroban runs in a `no_std` environment — no `serde_json`. Parse `clientDataJSON` with a simple byte-slice search (`memmem`-style) for `"origin":"`. Do not pull in a full JSON parser.
...
## Issue 2 — End-to-End Testnet Demo App
...
- `.env.example` (from Issue #5) — environment variable reference
...
## Issue 3 — SDK: Verify Wallet Exists On-Chain in `login()`
...
`login()` should accept `rpcUrl` and `networkPassphrase` from the hook's `WalletConfig` (already available via destructuring).
...
- The `getContractData` call is already implemented in `deploy()` — extract or replicate the pattern
...
## Issue 4 — Testnet Smoke Test Script
...
Without an automated smoke test, regressions in the factory integration or the `deploy()` polling logic will only be caught by hand. This slows down PR review and makes it harder to onboard contributors who want to verify their changes actually work.

## What needs to be done

Add a Node.js script at `scripts/smoke_test.ts` (run with `ts-node` or compiled to JS) that:
...
3. Submits a factory `deploy` transaction directly (replicating the `deploy()` hook logic without a browser) using the Stellar SDK
...
The script does not need to test the WebAuthn credential creation (that requires a browser). Use a hardcoded 65-byte mock P-256 public key as the signer.

## Key files

- `sdk/src/utils.ts` — `computeWalletAddress()` to use directly
- `sdk/src/useInvisibleWallet.ts` — `deploy()` logic to replicate (without the React hook wrapper)
- `scripts/deploy_factory.sh` — the factory contract ID output this script will read from

## Implementation notes

- Read `FACTORY_ADDRESS`, `RPC_URL`, and `NETWORK_PASSPHRASE` from environment variables (`.env.testnet` written by `deploy_factory.sh`)
- Use a hardcoded valid-format 65-byte P-256 public key (all zeros except the `0x04` prefix) — the factory only checks length, not cryptographic validity
- Exit with code 0 on success, non-zero on failure — makes it usable in CI later
- Add a `scripts/README.md` or comment block explaining how to run it

## Done when

- [ ] `scripts/smoke_test.ts` exists and runs with `npx ts-node scripts/smoke_test.ts`
- [ ] Script funds itself via Friendbot without manual steps
- [ ] Script asserts the wallet contract exists on-chain after deployment
- [ ] Exits 0 on success, non-zero on failure
- [ ] Reads config from env vars — no hardcoded contract IDs or RPC URLs
```

---

## Issue 5 — Environment Config and `.env.example`
...
Veil's SDK and upcoming demo app require several environment variables: the deployed factory contract address, an RPC URL, and the network passphrase. Contributors have no reference for what these should look like or where to get them.

## Problem

Every contributor who wants to run the demo or smoke test has to figure out the required environment variables from reading the source code. There is no `.env.example`, no setup guide, and no documented path from "fresh clone" to "running against testnet".

## What needs to be done

1. Add `.env.example` at the repo root with all variables needed across the project:
...
# Smoke test / scripts (no NEXT_PUBLIC_ prefix — Node only)
RPC_URL=https://soroban-testnet.stellar.org
NETWORK_PASSPHRASE=Test SDF Network ; September 2015
FACTORY_ADDRESS=                     # same as above
```

2. Add a `TESTNET_SETUP.md` at the repo root (or a new `docs/testnet-setup.md` section) covering:
   - How to install the Stellar CLI
   - How to create and fund a testnet account
...
- [ ] `TESTNET_SETUP.md` exists with clear step-by-step instructions
- [ ] `.env.local` and `.env.testnet` listed in `.gitignore`
- [ ] A contributor following the guide from a fresh clone can run the smoke test successfully
```
